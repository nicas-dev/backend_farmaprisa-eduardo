...
public partial class FarmaPrisaContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<DetallesPedido>(entity =>
        {
            ...
            entity.ToTable("detalles_pedido", tb => tb.HasComment("Tabla de cruce para saber qué productos van en cada pedido."));
        ...
        modelBuilder.Entity<FavoritosUsuario>(entity =>
        {
            ...
            entity.ToTable("favoritos_usuario", tb => tb.HasComment("Tabla para la gestión de los productos favoritos de los usuarios"));
        ...
        modelBuilder.Entity<InventarioSucursal>(entity =>
        {
            ...
            entity.ToTable("inventario_sucursal", tb => tb.HasComment("Tabla para el inventario por sucursal"));
            ...
            entity.HasOne(d => d.Producto).WithMany(p => p.InventarioSucursals)
                .HasForeignKey(d => d.ProductoId)
                .HasConstraintName("inventario_sucursal_ibfk_1");

            entity.HasOne(d => d.Sucursal).WithMany(p => p.InventarioSucursals)
                .HasForeignKey(d => d.SucursalId)
        ...
        modelBuilder.Entity<OpinionesProducto>(entity =>
        {
            entity.HasKey(e => e.Id).HasName("PRIMARY");

            entity.ToTable("opiniones_productos", tb => tb.HasComment("Tabla de Opiniones y Valoraciones"));
            ...
            entity.HasOne(d => d.Producto).WithMany(p => p.OpinionesProductos)
                .HasForeignKey(d => d.producto_id)
                .HasConstraintName("opiniones_productos_ibfk_1");

            entity.HasOne(d => d.Usuario).WithMany(p => p.OpinionesProductos)
                .HasForeignKey(d => d.UsuarioId)
                .IsRequired(false)
        ...
        modelBuilder.Entity<Producto>(entity =>
        {
            ...
            entity.ToTable("productos", tb => tb.HasComment("La tabla principal del catálogo de productos."));

            //entity.HasQueryFilter(p => p.EstaActivo == true);
            ...
            entity.Property(e => e.PuntosParaCanje)
                .HasColumnType("int(11)")
        ...
        // FarmaPrisaContext.cs (dentro de OnModelCreating)

        modelBuilder.Entity<ProductoDetalle>(entity =>
        {
            // 1. Configuración de la Tabla y Clave Primaria
            // Dejamos que la entidad use el [Key] de la clase o la convención para 'id'
            ...
            // Mapeo explícito de producto_id (FK)
            entity.Property(e => e.producto_id)
                .HasColumnType("int(11)")
                .HasColumnName("producto_id");

            // Mapeo explícito de tipo_detalle_id (FK)
            entity.Property(e => e.TipoDetalleId)
                .HasColumnType("int(11)")
                .HasColumnName("tipo_detalle_id");

            entity.Property(e => e.valor_detalle)
                .HasColumnName("valor_detalle");

            entity.Property(e => e.MediaUrl)
                .HasColumnName("media_url");

            // 3. Configuración de Relaciones (Claves Foráneas CORRECTAS)

            // FK a Productos (el producto principal)
            entity.HasOne(d => d.Producto)
                .WithMany(p => p.Detalles)
                .HasForeignKey(d => d.producto_id) // <--- ¡USAMOS producto_id!
                .IsRequired(false) // Mantenemos opcional para el filtro global
                .HasConstraintName("producto_detalle_ibfk_1");

            // FK a TipoDetalle (el catálogo de tipos)
            entity.HasOne(pd => pd.TipoDetalle)
                .WithMany(td => td.ProductoDetalles)
                .HasForeignKey(pd => pd.TipoDetalleId)
                .HasConstraintName("producto_detalle_ibfk_2");
        });

        modelBuilder.Entity<ProductoSintoma>(entity =>
        {
            entity.HasKey(ps => new { ps.ProductoId, ps.SintomaId });

            entity.ToTable("producto_sintomas", tb => tb.HasComment("Tabla para conectar los productos con sus síntomas."));

            entity.HasIndex(e => e.ProductoId, "producto_id");

            entity.HasIndex(e => e.SintomaId, "sintoma_id");

            entity.Property(e => e.ProductoId)
                .HasColumnType("int(11)")
                .HasColumnName("producto_id");
            entity.Property(e => e.SintomaId)
                .HasColumnType("int(11)")
                .HasColumnName("sintoma_id");

            entity.HasOne(d => d.Producto).WithMany(p => p.ProductoSintomas)
                .HasForeignKey(d => d.ProductoId)
                .IsRequired(false)
                .HasConstraintName("producto_sintomas_ibfk_1");

            entity.HasOne(d => d.Sintoma).WithMany(p => p.ProductoSintomas)
        ...
        modelBuilder.Entity<ProductoImagen>(entity =>
        {
            ...
            entity.HasOne(d => d.Producto)
                .WithMany(p => p.ImagenesGaleria) // El nombre de la colección en la entidad Producto
                .HasForeignKey(d => d.ProductoId)
        ...
        modelBuilder.Entity<PromocionProducto>(entity =>
        {
            ...
            entity.ToTable("promocion_productos", tb => tb.HasComment("Tabla para aplicar una promoción a productos específicos"));
